...
(async () => {
  console.log('🧪 Testing Registration Workflow UI...');
  ...
  try {
    ...
    await page.goto('https://phuketsummercamp.com/admin', { waitUntil: 'networkidle' });
    
    // Check if test login is available
    const testLogin = await page.locator('text=Use Test Login').first();
    if (await testLogin.isVisible()) {
      ...
      console.log('❌ Test login not available');
    }
